...
class ITAssetDecommissioning(Document):
    ...
    def autoname(self):
        ...
        series_key = "ASSET-DECOM-"

        self.sync_asset_decommissioning_series(series_key)
        series_number = getseries(series_key, 4)
        self.name = f"ASSET-DECOM-{date_part}-{series_number}"

    def sync_asset_decommissioning_series(self, series_key):
        max_existing = 0
        name_pattern = re.compile(r"-(\d+)$")

        for docname in frappe.get_all(
            self.doctype,
            filters={"name": ["like", "ASSET-DECOM-%"]},
            pluck="name",
        ):
